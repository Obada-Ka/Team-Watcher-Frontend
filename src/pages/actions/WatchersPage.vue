<template>
  ...
  <template v-if="!isLoading">
    <ul v-if="usersStatus.length">
      <user-item
        v-for="user in usersStatus"
        :key="user.id"
        :name="user.name"
...
export default defineComponent({
  ...
  computed: {
    filteredUser() {
      let filteredUsers = this.$store.getters["user/usersStatus"].filter(
        (user: any) => {
          return user.name
            .toLowerCase()
  ...
  watch: {
    searchStatus() {
      this.usersStatus = this.$store.getters["user/usersStatus"].filter(
        (user: any) => {
          return user.status === this.searchStatus;
        }
      );
      console.log("searchStatus", this.usersStatus);
    },
    searchTerm() {
      this.usersStatus = this.$store.getters["user/usersStatus"].filter(
        (user: any) => {
      ...
      console.log("searchTerm", this.usersStatus);
    },
  },
  methods: {
    async getUsersStatus() {
      this.isLoading = true;
      try {
        await this.$store.dispatch("user/getUsersStatus");
        this.usersStatus = this.$store.getters["user/usersStatus"];
